...
#exe("rm t1ewptall-s9.dat")
exe("rm t1ewptall-test-s9.dat")
...
#fin=open("t1ewptallhelp-s9.dat")
fin=open("t1ewptallhelp-test-s9.dat")
...
for i in range(0,len(inhelp)-1):
    ...
    #gammat4toH0 = ((mt4/(64.*pi))*(((-lamHt34)**2 + (-lamHrt34)**2)*(1. + (mtpole**2 -mHH**2)/mt4**2) + 4.*(lamHt34)*(lamHrt34)*mtpole/mt4)*sqrt(abs(phase(1.,(mHH/mt4)**2,(mtpole/mt4)**2))))*unitstep(mt4 - mHH - mtpole)
    ...
    #Neutral CP-even Higgs H
    #gammab4toHH = ((mb4/(64.*pi))*(((lamHb34)**2 + (lamHrb34)**2)*(1. + (mbpole**2 -mHH**2)/mb4**2) + 4.*(lamHb34)*(lamHrb34)*mbpole/mb4)*sqrt(abs(phase(1.,(mHH/mb4)**2,(mbpole/mb4)**2))))*unitstep(mb4 - mHH - mbpole)
    #Charged Higgs case:let's assume it H for simplicity
    if mb4 < mHH + mtpole:# + 1000:
        continue
